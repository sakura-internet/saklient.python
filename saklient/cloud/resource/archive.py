...
class Archive(Resource):
    ...
    # 
    # @return {saklient.cloud.resource.archive.Archive} this
    def save(self):
        return self._save()
    ...
    ## @return {saklient.cloud.resource.ftpinfo.FtpInfo}
        ...
        if r is not None:
            ...
            if ( "SourceDisk" in r if isinstance(r, dict) else hasattr(r, "SourceDisk")):
                ...
                if s is not None:
                    id = ( (s["ID"] if "ID" in s else None ) if isinstance(s, dict) else getattr(s, "ID"))
                    if id is not None:
                        obj = Util.create_class_instance("saklient.cloud.resource.Disk", [self._client, s])
                        self._source = obj
    ...
    # @param {(saklient.cloud.resource.archive.Archive, bool) => void} callback
    # @return {void}
    ...
    def set_size_mib(self, v):
        Util.validate_type(v, "int")
        if not self.is_new:
            raise SaklientException("immutable_field", "Immutable fields cannot be modified after the resource creation: " + "saklient.cloud.resource.archive.Archive#size_mib")
        self.m_size_mib = v
