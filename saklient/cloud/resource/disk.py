...
from saklient.cloud.resource.diskconfig import DiskConfig
from saklient.cloud.enums.eavailability import EAvailability
from saklient.cloud.enums.ediskconnection import EDiskConnection
...
class Disk(Resource):
    ...
    # 
    # @return {saklient.cloud.resource.disk.Disk} this
    def save(self):
        return self._save()
    ...
    ## @return {saklient.cloud.resource.resource.Resource}
    def get_source(self):
        return self._source
    
    ## @param {saklient.cloud.resource.resource.Resource} source
    # @return {saklient.cloud.resource.resource.Resource}
    def set_source(self, source):
        Util.validate_type(source, "saklient.cloud.resource.resource.Resource")
        self._source = source
        return source
        ...
        if r is not None:
            ...
            if ( "SourceArchive" in r if isinstance(r, dict) else hasattr(r, "SourceArchive")):
                ...
                if s is not None:
                    id = ( (s["ID"] if "ID" in s else None ) if isinstance(s, dict) else getattr(s, "ID"))
                    if id is not None:
                        obj = Util.create_class_instance("saklient.cloud.resource.Archive", [self._client, s])
                        self._source = obj
    ...
    ## ディスクをサーバに取り付けます。
    # 
    # @param {saklient.cloud.resource.server.Server} server
    # @return {saklient.cloud.resource.disk.Disk} this
    def connect_to(self, server):
        Util.validate_type(server, "saklient.cloud.resource.server.Server")
    ...
    # 
    # @return {saklient.cloud.resource.diskconfig.DiskConfig}
    ...
    # @param {(saklient.cloud.resource.disk.Disk, bool) => void} callback
    # @return {void}
    ...
    def set_size_mib(self, v):
        ...
        if not self.is_new:
            raise SaklientException("immutable_field", "Immutable fields cannot be modified after the resource creation: " + "saklient.cloud.resource.disk.Disk#size_mib")
        self.m_size_mib = v
    ...
    ## (This method is generated in Translator_default#buildImpl)
    # 
    # @return {saklient.cloud.resource.diskplan.DiskPlan}
    def get_plan(self):
        return self.m_plan
    
    ## (This method is generated in Translator_default#buildImpl)
    # 
    # @param {saklient.cloud.resource.diskplan.DiskPlan} v
    # @return {saklient.cloud.resource.diskplan.DiskPlan}
    def set_plan(self, v):
        Util.validate_type(v, "saklient.cloud.resource.diskplan.DiskPlan")
        if not self.is_new:
            raise SaklientException("immutable_field", "Immutable fields cannot be modified after the resource creation: " + "saklient.cloud.resource.disk.Disk#plan")
        self.m_plan = v
        self.n_plan = True
        return self.m_plan
    ...
    ## (This method is generated in Translator_default#buildImpl)
    # 
    # @return {saklient.cloud.resource.server.Server}
    def get_server(self):
