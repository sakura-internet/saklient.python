...
class Model:
    ...
    def _get_by_id(self, id):
        ...
        self._total = 1
        self._count = 1
        return Util.create_class_instance("saklient.cloud.resources." + self._class_name(), [self._client, result, True])
    ...
    def _find_one(self):
        ...
        if self._total == 0:
            return None
        records = ( (result[self._root_key_m()] if self._root_key_m() in result else None ) if isinstance(result, dict) else getattr(result, self._root_key_m()))
        return Util.create_class_instance("saklient.cloud.resources." + self._class_name(), [self._client, records[0]])
