...
class CdromIsIncompleteException(HttpConflictException):
    ...
    def __init__(self, status, code=None, message=""):
        super(CdromIsIncompleteException, self).__init__(status, code, "要求された操作を行えません。このISOイメージは不完全です。複製処理等の完了後に再度お試しください。" if message is None or message == "" else message)
